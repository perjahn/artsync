...
func Generate(
	...
	for _, repo := range repos {
		if onlyGenerateMatchingRepos && !filterMatchingRepos(repo.Key, permissiondetails) {
			continue
		}
		...
		if allowRenamedPermissions {
			var filter bool
			permissionName, filter = filterRenamedRepos(repo.Key, permissiondetails)
			if !filter {
				continue
			}
		}

		if onlyGenerateCleanRepos && !filterCleanRepos(repo.Key, permissiondetails, useAllPermissionTargetsAsSource) {
			continue
		}
...
func filterMatchingRepos(repokey string, permissiondetails []ArtifactoryPermissionDetails) bool {
	for _, permission := range permissiondetails {
...
func filterRenamedRepos(repokey string, permissiondetails []ArtifactoryPermissionDetails) (string, bool) {
	for _, permission := range permissiondetails {
		if permission.Name == repokey {
			return "", true
		}
...
func filterCleanRepos(repokey string, permissiondetails []ArtifactoryPermissionDetails, useAllPermissionTargetsAsSource bool) bool {
	if useAllPermissionTargetsAsSource {
		for _, permission := range permissiondetails {
			for reponame := range permission.Resources.Artifact.Targets {
				if reponame == repokey {
