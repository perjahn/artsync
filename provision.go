...
func Provision(
	...
	allowpatterns bool,
	createUsers bool,
	importLdapGroups bool,
	dryRun bool) error {

	fmt.Printf("Repos to provision: %d\n", len(reposToProvision))
	for _, repo := range reposToProvision {
		err := validateRepo(repo, allusers, allgroups, createUsers, importLdapGroups)
		if err != nil {
...
func validateRepo(
	...
	for _, check := range []struct {
		...
		errors := checkUsersAndGroups(check.values, allusers, allgroups, createUsers, importLdapGroups)
		if len(errors) > 0 {
